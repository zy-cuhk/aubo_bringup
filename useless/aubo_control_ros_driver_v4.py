...
class AuboRosDriver():
    ...
    def Init_aubo_driver(self,Aubo_IP,maxacctuple,maxvelctuple,ee_maxacc,ee_maxvelc,blend_radius):
        ...
        try:
            ...
            if result != RobotErrorType.RobotError_SUCC:
                ...
                #set joint max acc
                self.robot.set_joint_maxacc((2.5, 2.5, 2.5, 2.5, 2.5, 2.5))#(joint_maxacctuple)#(2.5, 2.5, 2.5, 2.5, 2.5, 2.5)
                #set joint max vel
                self.robot.set_joint_maxvelc((1.5, 1.5, 1.5, 1.5, 1.5, 1.5))#(joint_maxvelctuple)#(1.5, 1.5, 1.5, 1.5, 1.5, 1.5)
                #set ee max acc
                rospy.logerr("wocao-------")
                print("fuck this aubo-----%s----",str(self.robot.set_end_max_line_acc(0.2)))
                
                #set ee max vel
                self.robot.set_end_max_line_velc(0.3)
    ...
    def Aubo_trajectory_init(self,joint_maxacctuple,joint_maxvelctuple,ee_maxacc,ee_maxvelc,blend_radius):
        ...
        #set ee max acc
        # rospy.logerr("wocao-------")
        # rospy.logerr("fuck this aubo-----%s----",str(self.robot.set_end_max_line_acc(0.2)))
...
def main():
    ...
    ee_maxacc=0.2#rospy.get_param('ee_maxacc')      #Aub.Tuple_string_to_tuple(rospy.get_param('ee_maxacc'))
    ee_maxvelc=0.2#rospy.get_param('ee_maxvelc')     #Aub.Tuple_string_to_tuple(rospy.get_param('ee_maxvelc'))
